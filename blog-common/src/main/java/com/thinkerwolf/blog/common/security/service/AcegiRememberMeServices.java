 ...
 * 
 */
public class AcegiRememberMeServices implements RememberMeServices, InitializingBean, LogoutHandler {

	public static final String ACGEI_REMEMBERME_COOKIE_NAME = "_acegi_rememberme_cookie_name";

	private static final Logger logger = InternalLoggerFactory.getLogger(AcegiRememberMeServices.class);
	/**
	 * 存储在Cookie中的key
	 */
			...
			if (cookieName.equals(cookie.getName())) {
				...
				if (Base64.isArrayByteBase64(cookieValue.getBytes())) {
					...
					if (cookies.length == 3) {
						long validityTime;
						try {
							validityTime = new Long(cookiesDetails[2]).longValue();
						} catch (Exception e) {
						...
						UserDetails userDetails = loadUserDetails(request, response, cookiesDetails);
	...
	public void loginSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		Assert.notNull(successfulAuthentication.getPrincipal(), "principal is null");
		Assert.notNull(successfulAuthentication.getCredentials(), "credential is null");
	...
	protected boolean rememberRequest(HttpServletRequest request, String parameter) {
